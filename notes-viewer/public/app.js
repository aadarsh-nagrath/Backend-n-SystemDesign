...
async function fetchNotes() {
  try {
    ...
    // Sort: Index.md or README.md always first, rest alphabetically
    allNotes.sort((a, b) => {
      const aIsIndex = (a.name.toLowerCase() === 'index.md' || a.name.toLowerCase() === 'readme.md') && !a.path.includes('/');
      const bIsIndex = (b.name.toLowerCase() === 'index.md' || b.name.toLowerCase() === 'readme.md') && !b.path.includes('/');
    ...
    // Pin README.md / Index.md above the tree and exclude it from the regular list
    const indexNote = allNotes.find(n => (n.name.toLowerCase() === 'index.md' || n.name.toLowerCase() === 'readme.md') && !n.path.includes('/'));
    if (indexNote) {
      indexPinEl.href = `#${encodeURIComponent(indexNote.path)}`;
      indexPinEl.dataset.path = indexNote.path;
      indexPinEl.querySelector('span').textContent = indexNote.name.replace(/\.md$/i, '');
    ...
    // Handle URL Hash navigation, or open Index/README by default
...
function handleHashNavigation() {
  ...
  // Default: open root Index.md or README.md
  const indexNote = allNotes.find(n => (n.name.toLowerCase() === 'index.md' || n.name.toLowerCase() === 'readme.md') && !n.path.includes('/'));
  if (indexNote) {
    loadNote(indexNote.path);
  }
...
// Matched against both the filename and the full lowercased path.
const ICON_RULES = [
  // --- Roadmap & Guides ---
  [/roadmap|guide/, 'fa-solid fa-route', 'icon-roadmap'],
  ...
  // --- System Design ---
  [/system.?design|architecture|distributed|microservices|load.?balancer|websocket|ipc/, 'fa-solid fa-diagram-project', 'icon-sysdesign'],

  // --- APIs ---
  [/api-security|web-security|security|oauth|jwt|auth|authentication|token|cors|csrf|xss/, 'fa-solid fa-shield-halved', 'icon-security'],
  [/api|rest|graphql|grpc|rpc|endpoints|payload|http/, 'fa-solid fa-plug', 'icon-api'],

  // --- Web Servers & Proxies ---
  [/web-servers|server|nginx|apache|reverse.?proxy|gateway/, 'fa-solid fa-server', 'icon-server'],

  // --- Git ---
  [/git|github/, 'fa-solid fa-code-branch', 'icon-git'],

  // --- Docs / Index ---
  [/readme|index/, 'fa-solid fa-book', 'icon-index'],
];
...
function getFileIconClass(fileName, filePath) {
  ...
  if (name === 'index.md' || name === 'readme.md') return 'fa-solid fa-book icon-index';
  ...
  if (name.endsWith('.json')) return 'fa-solid fa-file-code icon-json';
